...
const Open = ({ data, index }: Props) => {
  ...
  const getActionType = (attributes: Array<any>) => {
    ...
    if (attributes?.length) {
      if (attributes[0]?.trait_type === 'Perk Type') {
        type = attributes[0]?.value;
      }
      return `${type} Capsule`;
    } else {
      return 'Origin Capsule';
    }
  };
  return (
    <TableRow
    ...
    >
      ...
      >
        ...
        >
          <img
            src="/images/activityPanel/gameloop/OPEN.svg"
            width={25}
            height={25}
          />
          <Box
          ...
          >
            <Link target={'_blank'} href="/gameloop/gamification/capsules">
              <Typography
                variant="body1"
                sx={{
                  'fontWeight': 700,
