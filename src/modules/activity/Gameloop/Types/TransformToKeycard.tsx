...
const TransformToKeycard = ({ data, index }: Props) => {
  ...
  const getActionType = (attributes: Array<any>) => {
    let type = '';
    let tier = '';
    if (attributes[0]?.trait_type === 'IsDirectBurnKey') {
      type = attributes[0]?.value === 1 ? 'Origin' : 'Drift';
    }
    if (attributes[2]?.trait_type === 'IsOdinKey') {
      tier = attributes[2]?.value === 1 ? 'Odin' : 'Thor';
    }
    return `${type} ${tier} Node`;
  };
  return (
    <TableRow
      sx={{
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <Link target={'_blank'} href="/gameloop/transform2keycard">
              <Typography
                variant="body1"
                sx={{
                  'fontWeight': 700,
...
export default TransformToKeycard;
